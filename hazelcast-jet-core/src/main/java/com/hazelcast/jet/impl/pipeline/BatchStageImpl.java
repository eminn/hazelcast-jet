...
public class BatchStageImpl<T> extends ComputeStageImplBase<T> implements BatchStage<T> {
    ...
    @Nonnull
    @Override
    public <C, R> GeneralStage<R> mapUsingContextAsync(@Nonnull ContextFactory<C> contextFactory, @Nonnull DistributedBiFunction<? super C, ? super T, CompletableFuture<? extends Traverser<? extends R>>> callAsyncFn) {
        return attachMapUsingContextAsync(contextFactory,callAsyncFn);
    }
